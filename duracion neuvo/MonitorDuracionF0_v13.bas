...
'==================================================================
'  DURACION FONDO 0  -  v13
'  Base = v10 (funcionaba). Cambios minimos:
'  - fCorte: fecha de corte robusta desde la FMS (ToFecha), usada
'    en el calculo de dias hasta vencimiento (antes CDate(dFMS)
'    podia fallar si dFMS venia como texto AAAAMMDD).
'  - Flujos: lectura de fecha robusta (acepta fecha real o AAAAMMDD)
'    + MesCorto blindado + excluye grupos "OTRO:". (arregla error 13)
'  - Quitado el cartelito de diagnostico del paso 10.
...
Private Const C_CANT As Long = 10, C_RATING As Long = 11, C_VCTO As Long = 12
' --- Anexo I ---
...
' --- Hoja de Depositos ---
Private Const DEP_SBS As Long = 3, DEP_YTM As Long = 6
...
Sub CalcularDuracionFondo0()
    ...
    ' Robusta: usa ToFecha para convertir el texto AAAAMMDD sin depender de CDate.
    Dim fCorte As Date
    Dim fc As Variant: fc = ToFecha(dFMS)
    If IsDate(fc) Then fCorte = CDate(fc) Else fCorte = hoy
    ...
    Dim depYTM As Object: Set depYTM = LeerDepositos(wbHome)
    ...
    If lastA >= 2 Then
        ...
        For i = 1 To UBound(datA, 1)
            If NormFond(datA(i, A_FOND)) = "9" Then
    ...
    If Not IsEmpty(datC) Then
        For i = 1 To UBound(datC, 1)
            If NormFond(datC(i, C_FOND)) = "9" Then
                nInstr = nInstr + 1
                ...
                Else
                    ...
                    If IsDate(vd) And nC >= 1 Then
                        ...
                        If depYTM.Exists(cod) Then ytw = depYTM(cod) Else ytw = ""
                        spr = "": origen = "Deposito (curva)"
                        nCup = nCup + 1
                        cpNemo(nCup) = CStr(datC(i, C_NEMO)): cpCod(nCup) = cod
                        cpVcto(nCup) = CDate(vd): cpDias(nCup) = diasB: cpDur(nCup) = dur
                    Else
                        ytw = "": spr = "": dur = "NO CALCULABLE": origen = "Cupon sin vcto/curva"
                        nSinDur = nSinDur + 1
                    End If
                End If
...
Private Sub LeerConfig(wb As Workbook, ByRef cFMS As String, ByRef cVec As String, _
    ...
    ws.Range("A4").Value = "Fecha Vector (AUTO = t-1, o escribe fecha):"
...
Private Function BestDateCode(dVec As String, dFMS As String) As String
    If Len(dVec) = 8 Then
     BestDateCode = dVec
    ElseIf Len(dFMS) = 8 Then
     BestDateCode = dFMS
    Else
    BestDateCode = Format(Now, "yyyymmdd")
    End If
End Function

Private Function LeerDepositos(wb As Workbook) As Object
  Dim d As Object: Set d = CreateObject("Scripting.Dictionary")
  Dim ws As Worksheet
  On Error Resume Next
  Set ws = wb.Worksheets("Depositos")
  On Error GoTo 0
  If ws Is Nothing Then Set LeerDepositos = d: Exit Function
  Dim last As Long, i As Long, cod As String, tasa As Variant
  last = ws.Cells(ws.Rows.Count, DEP_SBS).End(xlUp).Row
  For i = 2 To last
     cod = NormCode(ws.Cells(i, DEP_SBS).Value)
     If Len(cod) > 0 And Not d.Exists(cod) Then
        tasa = ws.Cells(i, DEP_YTM).Value
        d(cod) = tasa
     End If
 Next i
 Set LeerDepositos = d
...
Private Function GrupoAC(ByVal ac As String) As String
    ...
    Select Case s
        Case "bonos de gobierno central de la republica", _
             "bonos de gobierno central de la rep" & ChrW(250) & "blica", _
             "bonos de empresas privadas", _
             "otros bonos del sistema financiero", _
             "titulos con derecho crediticio", _
             "t" & ChrW(237) & "tulos con derecho crediticio"
            GrupoAC = "Bonos"
        Case "depositos a plazo", "dep" & ChrW(243) & "sitos a plazo"
